...
const checkPassword = () => {
  ...
  if (passwordInput.value === passwordCheckInput.value) {
    checkParagraph.textContent = "Passwords match.";
    document.querySelector(".error-info").appendChild(checkParagraph);
...
submitButton.addEventListener("click", (event) => {
  ...
  if (passwordCheck.length === 0) {
    isValid = false;
    document.getElementById("password-check").style.border = "2px solid red";
  ...
  if (isValid) {
    ...
    if (!isValid) {
      infoParagraph.textContent =
        "Your information were not registered. Try again.";
      document.querySelector(".send-info").appendChild(infoParagraph);
    }
  }
});
